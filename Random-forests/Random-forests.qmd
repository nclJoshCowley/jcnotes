...
$$
    \hat{h}_{\text{RF}}(x)
        = \frac{1}{q} \sum_{l=1}^q \hat{h}(x, \Theta_l)
$$
where $\hat{h}(\cdot, \Theta_1), \dots, \hat{h}(\cdot, \Theta_q)$ are a
collection of tree predictors with $q$ i.i.d random variables independent
